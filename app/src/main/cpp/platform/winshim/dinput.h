// dinput.h — DirectInput shim (Re-Volt Android port).
// Input.cpp / ctrlread.cpp get REPLACED by Android touch/gamepad input, so
// this only carries the types/constants their HEADERS mention.
#pragma once
#include "windows.h"

// opaque interfaces — forward-declared so `IDirectInputDevice2 *x` also works
struct IDirectInput;        struct IDirectInput2;
struct IDirectInput7;       struct IDirectInputDevice;
struct IDirectInputDevice2; struct IDirectInputDevice7;
...
} DIDEVICEINSTANCEA, DIDEVICEINSTANCE, *LPDIDEVICEINSTANCE;
...
} DIDEVICEOBJECTINSTANCEA, DIDEVICEOBJECTINSTANCE, *LPDIDEVICEOBJECTINSTANCE;
...
// keyboard scan codes (DIK_*) — kept because the game's key tables use them;
// the Android input layer will translate to these.
...
// alternate names (as in the real dinput.h)
